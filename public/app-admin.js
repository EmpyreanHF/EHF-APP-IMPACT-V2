            ...
            window._adminDeleteNewsArticle = function(docId) {
                ...
                window.fbDb.collection('news_articles').doc(docId).delete()
                    .then(function() {
                        ...
                        _refreshNewsTable();
                        /* Remove from in-memory dashboardNews so it disappears from the news feed */
                        if (window.dashboardNews) {
                            window.dashboardNews = window.dashboardNews.filter(function(a) { return a.id !== docId; });
                ...
                form.addEventListener('submit', async function(e) {
                    ...
                    try {
                        ...
                        if (window._firebaseLoaded && window.fbDb) {
                            await window.fbDb.collection('news_articles').doc(article.id).set(article);
                        }

                        /* Inject into in-memory dashboardNews so feed updates live */
                        if (!window.dashboardNews) window.dashboardNews = [];
                        window.dashboardNews.unshift(article);
                        if (typeof window.renderDashboardNews === 'function') window.renderDashboardNews();
