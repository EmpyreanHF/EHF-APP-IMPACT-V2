...
(function empyreanBusinessModule() {
    ...
    function renderBusinessPage() {
        ...
        var us  = _us();
        /* BUG FIX: Bridge app-fix-final.js click handlers (which set window._activeBizData)
           into window._viewingBizPage so this renderer always picks up the correct page.
           app-fix-final.js uses _activeBizData; app-business.js uses _viewingBizPage —
           sync them here so both systems stay consistent. */
        if (window._activeBizData && !window._viewingBizPage) {
            window._viewingBizPage = window._activeBizData;
        }
        var biz = window._viewingBizPage || us.businessPage;
        /* If viewing someone else's page, mark it so the UI adapts */
        var _isOwnPage = !window._viewingBizPage || (biz && biz.ownerId === us.id) || (biz && biz.id && us.businessPage && biz.id === us.businessPage.id);
        ...
        + (_isOwnPage ? '<i class="fas fa-pen edit-icon" data-field="name" title="Edit name" style="font-size:0.65rem;color:#9CA3AF;cursor:pointer;"></i>' : '')
        + '</h2>'
        + '<p id="business-page-tagline" style="color:#6B7280;font-size:0.92rem;margin:0 0 8px;'
        + 'display:flex;align-items:center;gap:8px;">'
        + _esc(biz.tagline || '')
        + (_isOwnPage ? '<i class="fas fa-pen edit-icon" data-field="tagline" title="Edit tagline" style="font-size:0.6rem;color:#9CA3AF;cursor:pointer;"></i>' : '')
        + '</p>'
        ...
        + (_isOwnPage
            ...
            : '<button class="btn btn-accent biz-visitor-follow-btn" data-biz-id="' + _esc(biz.id) + '" '
              + 'style="border-radius:50px;padding:9px 24px;font-size:0.85rem;font-weight:700;">'
        ...
        + (_isOwnPage ? '<i class="fas fa-pen edit-icon" data-field="email" style="font-size:0.6rem;color:#9CA3AF;cursor:pointer;"></i>' : '')
        + '</span>'
        ...
        + (_isOwnPage ? '<i class="fas fa-pen edit-icon" data-field="phone" style="font-size:0.6rem;color:#9CA3AF;cursor:pointer;"></i>' : '')
        + '</span>'
        ...
        + (_isOwnPage ? '<i class="fas fa-pen edit-icon" data-field="address" style="font-size:0.6rem;color:#9CA3AF;cursor:pointer;"></i>' : '')
        + '</span>'
        + '</div>'
        ...
        /* RIGHT COLUMN — post compose (owner only) + feed */
        + '<div style="display:flex;flex-direction:column;gap:16px;">'

        /* Post compose card — owner only */
        + (_isOwnPage ? '<div class="card" style="padding:20px;">'
        ...
        + '<p style="margin:0;font-size:0.9rem;">' + (_isOwnPage ? 'No posts yet — share your first update above!' : 'No posts yet.') + '</p>'
        + '</div>'
        + '</div>'
        ...
        + '}'
        /* Business post feed scrollable */
        + '#business-page-feed-container{'
        + 'display:flex;flex-direction:column;gap:14px;'
        + 'max-height:none;overflow-y:visible;'
        + '}'
        /* Post compose area */
        + '#create-business-post-form textarea{'
        + 'min-height:80px;'
        + '}'
        /* Business post media preview horizontal scroll */
        + '#business-post-media-preview{'
        + 'display:flex;flex-direction:row;flex-wrap:nowrap;'
        + 'overflow-x:auto;gap:8px;padding-bottom:4px;'
        + 'scrollbar-width:none;-webkit-overflow-scrolling:touch;'
        + '}'
        + '#business-post-media-preview::-webkit-scrollbar{display:none;}'
        + '#business-post-media-preview img,'
        + '#business-post-media-preview video{'
        + 'flex:0 0 90px;width:90px;height:90px;'
        + 'object-fit:cover;border-radius:10px;'
        + 'border:2px solid rgba(27,43,139,0.2);'
        + '}'
        + '</style>';
    ...
    function renderDashboardBusinesses() {
        ...
        pages.slice(0, 8).forEach(function (biz) {
            ...
            /* Store full biz data as JSON on card for reliable lookup on click */
            try { card.dataset.bizData = JSON.stringify(biz); } catch(e3) {}
            card.title           = biz.name;
            card.style.cssText   =
            ...
            /* BUG FIX: Store the specific page being viewed so renderBusinessPage
               renders THAT business's page, not always the logged-in user's own page.
               Each card captures its own `biz` object in the closure. */
            card.addEventListener('click', (function(bizPage) {
                return function () {
                    window._viewingBizPage = bizPage;
                    _nav('business-page');
                    /* renderBusinessPage fires on navigateTo, but call explicitly
                       to ensure it reads the newly set _viewingBizPage */
                    setTimeout(function() {
                        if (typeof window.renderBusinessPage === 'function') {
                            window.renderBusinessPage();
                        }
                    }, 80);
                };
    ...
    function _handleBusinessPostSubmit(form) {
        ...
        (async function () {
            try {
                /* Upload to Cloudinary — treat each file independently */
                var cloudUrls = [];
                if (files.length > 0) {
                    if (typeof window.uploadMediaFilesToCloudinary === 'function') {
                        try {
                            cloudUrls = await window.uploadMediaFilesToCloudinary(files);
                        } catch(uploadErr) {
                            console.warn('[EmpBusiness] Cloudinary batch upload error:', uploadErr);
                            /* Try one-by-one fallback */
                            for (var fi = 0; fi < files.length; fi++) {
                                try {
                                    if (typeof window.uploadToCloudinary === 'function') {
                                        var u = await window.uploadToCloudinary(files[fi], null);
                                        cloudUrls.push(u || '');
                                    }
                                } catch(e2) { cloudUrls.push(''); }
                            }
                        }
                    } else if (typeof window.uploadToCloudinary === 'function') {
                        for (var fj = 0; fj < files.length; fj++) {
                            try { cloudUrls.push(await window.uploadToCloudinary(files[fj], null) || ''); }
                            catch(e3) { cloudUrls.push(''); }
                        }
                    }
                ...
                /* Prepend to business page feed immediately */
                var bizFeed = document.getElementById('business-page-feed-container');
                if (bizFeed && newPost) {
                    var placeholder = bizFeed.querySelector('[style*="No posts yet"]');
                    if (placeholder) placeholder.remove();
                    bizFeed.prepend(newPost);
                }

                /* Mirror to community dashboard feed — REMOVED.
                   Business posts must NOT appear in #posts-feed / #feed-container
                   (the general home-page feed). They live only inside
                   #business-page-feed-container on the Business Page section.
                   The "Business Posts" horizontal slider (#dashboard-business-container)
                   already provides the dashboard preview — no vertical injection needed.
                */

                /* Reset form immediately — don't await Firestore */
                form.reset();
                window._bizPostMediaFiles = [];
                var preview = form.querySelector('#business-post-media-preview');
                if (preview) preview.innerHTML = '';
                if (submitBtn) submitBtn.disabled = false;
                _notify('✅ Posted to your page!', 'success');

                /* Save to Firestore in background — failure never blocks UI */
                if (newPost && us.businessPage && window.fbDb) {
                    var validUrls = cloudUrls.filter(Boolean);
                    window.fbDb.collection('business_posts').doc(newPost.dataset.postId).set({
                        id:        newPost.dataset.postId,
                        userId:    us.id,
                        pageId:    us.businessPage.id,
                        pageName:  us.businessPage.name || '',
                        text:      text,
                        media:     validUrls,
                        createdAt: new Date().toISOString()
                    }).then(function(){
                        console.log('[EmpBusiness] Post saved to Firestore.');
                    }).catch(function(e) {
                        /* Upload was successful — only Firestore save failed */
                        console.warn('[EmpBusiness] Firestore post save failed (media already uploaded):', e && e.message);
                    });
                }

            } catch (err) {
                console.error('[EmpBusiness] Post submit error:', err);
                _notify('Failed to publish post: ' + (err.message || 'Try again'), 'error');
                if (submitBtn) submitBtn.disabled = false;
            }
        }());
    }
    ...
    function _bootstrap() {
        ...
        document.addEventListener('click', function (e) {
            ...
            /* Dashboard business card click → navigate to THAT business's page */
            var bizCard = closest('.dashboard-business-card');
            if (bizCard) {
                e.preventDefault();
                var bizId = bizCard.dataset.bizId;
                /* Find the page data from cache or live data */
                var allPages = (window._firestoreBusinessPages || []);
                var us2 = _us();
                if (us2.businessPage) allPages = allPages.concat([us2.businessPage]);
                var targetPage = allPages.find(function(p) { return p.id === bizId; });
                if (!targetPage && us2.businessPage && us2.businessPage.id === bizId) {
                    targetPage = us2.businessPage;
                }
                /* Also check demo/fallback pages by id */
                if (!targetPage) {
                    /* Try reading from the card's stored biz data attribute */
                    try { targetPage = JSON.parse(bizCard.dataset.bizData || 'null'); } catch(e2) {}
                }
                if (targetPage) window._viewingBizPage = targetPage;
                _nav('business-page');
                setTimeout(function() {
                    if (typeof window.renderBusinessPage === 'function') window.renderBusinessPage();
                }, 80);
                return;
            }
